...
工程ごとに確認を挟まず、要件定義から実装まで一気に通したい場合は `nonstop` を使う。各工程の確認事項・未決事項を既定案で仮確定して走り切り、完走後に決定ログ（`*.decisions.md`）でまとめて提示する。差し戻しは番号（`D-n`）で行う。

## オンランプ（途中から入る）

- バグ・単発の修正依頼（タスクリスト不要） → `implement` の修正モード
- 対話で要件を詰めたい → `requirements-definition-draft` の壁打ちモード
- 実装済みの差分から手動確認シナリオを起こす（仕様書が無くてもよい） → `test-scenario-from-diff`
